...
const EventForm: React.FC<EventFormProps> = ({ mode = 'create' }) => {
  ...
  // Tracks whether we're waiting for the organizer to add ticket types
  const [createStep, setCreateStep] = useState<CreateStep>({ step: 'idle' });
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === 'create') {
        // createEvent must return OrganizerEventOut so we can pass it to the
        // ticket types modal. Make sure your eventService is typed accordingly.
  ...
  // ── Ticket modal callbacks ─────────────────────────────────────────────────
  const handleTicketsFinished = (
    _event: OrganizerEventOut,
    ticketTypes: SavedTicketType[],
  ) => {
    setCreateStep({ step: 'idle' });
    // Navigate with a success flag so EventsList can show a toast if desired
    navigate('/events', {
      state: {
        flash: `Event created with ${ticketTypes.length} ticket ${ticketTypes.length === 1 ? 'type' : 'types'}.`,
      },
    });
  };

  const handleTicketsSkipped = (_event: OrganizerEventOut) => {
    setCreateStep({ step: 'idle' });
    navigate('/events', {
      state: { flash: 'Event created. Remember to add ticket types before it can accept bookings.' },
    });
  };

  // Closing the modal mid-flow = skip (event is already saved on the backend)
  const handleModalClose = () => {
    if (createStep.step === 'tickets') {
      handleTicketsSkipped(createStep.event);
    }
  };
  ...
  return (
    <>
      ...
      {/* Step 2: ticket types modal — rendered outside the page scroll container */}
      {createStep.step === 'tickets' && (
        <OrganizerCreateTicketTypesModal
          event={createStep.event}
