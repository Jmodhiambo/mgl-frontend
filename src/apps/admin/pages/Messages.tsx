...
// All categories from both the user form and the organizer form
const CATEGORY_OPTS = [
  'all',
  // User form categories
  'general',
  'support',
  'billing',
  'refund',
...
const SOURCE_OPTS = ['all', 'user', 'organizer'];
...
const Messages: React.FC = () => {
  ...
  const filtered = useMemo(() => messages.filter(m => {
    ...
    return true;
  }), [messages, search, statusFilter, catFilter, sourceFilter]);
  ...
  const sourceBadge = (source: string) =>
    source === 'organizer'
      ? <span className="badge-blue capitalize">Organizer</span>
      : <span className="badge-gray capitalize">User</span>;

  const hasActiveFilters = statusFilter !== 'all' || catFilter !== 'all' || sourceFilter !== 'all';
    ...
    <div className="space-y-5">
      ...
      <FilterBar
        ...
        actions={
          hasActiveFilters && (
            <button
              onClick={() => { setStatus('all'); setCat('all'); setSource('all'); }}
                ...
                <tbody>
                  {paginated.map(m => (
                    <tr key={m.id} className={m.status === 'new' ? 'bg-orange-50/30' : ''}>
                      ...
                      <td><span className="badge-gray capitalize">{formatCategory(m.category)}</span></td>
              ...
              {paginated.map(m => (
                <div key={m.id} className={`p-4 space-y-2 ${m.status === 'new' ? 'bg-orange-50/40' : ''}`}>
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="badge-gray capitalize">{formatCategory(m.category)}</span>
  ...
  const menu = open && createPortal(
    <>
      ...
      >
        ...
        {s === 'new' && (
          <button
            onClick={() => { onAction('pending'); setOpen(false); }}
        ...
        {(s === 'new' || s === 'pending') && (
          <button
            onClick={() => { onAction('responded'); setOpen(false); }}
        ...
        {s !== 'spam' && (
          <button
            onClick={() => { onAction('spam'); setOpen(false); }}
        ...
        {(s === 'closed' || s === 'spam') && (
          <button
            onClick={() => { onAction('new'); setOpen(false); }}
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-panel max-w-2xl p-6" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          <span className="badge-gray capitalize">{formatCategory(message.category)}</span>
          {message.source === 'organizer'
            ? <span className="badge-blue">Organizer</span>
            : <span className="badge-gray">User</span>
          }
          {/* event_title — only shown when present (organizer messages) */}
          {message.event_title && (
            <span className="inline-flex items-center gap-1 text-xs font-medium text-purple-700 bg-purple-50 border border-purple-200 rounded px-2 py-0.5">
        ...
        <div className="flex gap-2 pt-4 border-t border-gray-100 flex-wrap">
          ...
          {s === 'new' && (
            <button
              onClick={() => { onAction('pending'); onClose(); }}
          ...
          {(s === 'new' || s === 'pending') && (
            <button
              onClick={() => { onAction('responded'); onClose(); }}
          ...
          {s !== 'closed' && s !== 'spam' && (
            <button
              onClick={() => { onAction('closed'); onClose(); }}
